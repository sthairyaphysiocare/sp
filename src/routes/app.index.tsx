...
function KPI({
  icon: Icon, label, value, accent, active, onClick, mobilePanel,
}: {
  icon: any; label: string; value: number; accent: string;
  active: boolean; onClick: () => void; mobilePanel?: ReactNode;
}) {
    ...
    <div className={active ? "sm:contents" : ""}>
      ...
      >
        <div className="flex items-center justify-between gap-3">
          ...
          <div className={`size-11 sm:size-12 rounded-xl grid place-items-center text-white shrink-0 ${accent}`}>
...
function Dashboard() {
  ...
  function panelFor(k: KPIKey) {
    ...
    if (k === "today") return (
      <DataTable
        ...
        cols={["Patient", "Date", "Time", "Therapist"]}
        rows={todaysVisits.map((v) => {
          const p = patients.find((x) => x.id === v.patientId);
          return [p?.n ?? "Unknown", fmtDate(v.nxt), v.nxtTm || "—", v.tN];
        })}
  ...
  return (
    <div>
      ...
      <div className={`mt-8 grid sm:grid-cols-2 ${cards.length === 4 ? "lg:grid-cols-4" : "lg:grid-cols-2"} gap-4`}>
        {cards.map((c) => (
          <KPI
            key={c.key}
            icon={c.icon} label={c.label} value={c.value} accent={c.accent}
            active={open === c.key}
        ...
        <div className="p-6 rounded-2xl bg-card border">
          ...
          <div className="mt-4 space-y-2">
            {[...patients].slice(-5).reverse().map((p) => (
              <Link key={p.id} to="/app/patients/$id" params={{ id: p.id }} className="flex items-center justify-between px-3 py-3 rounded-lg hover:bg-accent gap-3">
                <div className="min-w-0">
                  <div className="font-medium truncate">{p.n}</div>
                  <div className="text-xs text-muted-foreground truncate">{p.pid} · {p.cc}</div>
                </div>
        ...
        <div className="p-6 rounded-2xl bg-card border">
          ...
          <div className="mt-4 space-y-2">
            ...
            {[...bookings].slice(-5).reverse().map((b) => (
              <div key={b.id} className="flex items-center justify-between px-3 py-3 rounded-lg bg-surface gap-3">
                <div className="min-w-0">
                ...
                <span className="text-xs px-2 py-1 rounded-full bg-orange-500/10 text-orange-700 capitalize shrink-0">{b.status}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="-mx-3 sm:mx-0 overflow-x-auto">
          <table className="w-full text-sm min-w-[420px]">
            <thead className="text-xs uppercase tracking-wider text-muted-foreground border-b">
              <tr>{cols.map((c) => <th key={c} className="py-2 px-3 text-left font-semibold whitespace-nowrap">{c}</th>)}</tr>
            </thead>
            <tbody>
              {rows.map((r, i) => (
                <tr key={i} className="border-b last:border-0 hover:bg-accent/40">
                  {r.map((cell, j) => (
                    <td key={j} className="py-2.5 px-3 align-top">
                      {(j === 0) && links?.[i] ? (
                        <Link to={links[i]} className="text-brand font-medium hover:underline">{cell}</Link>
                      ) : cell}
                    </td>
                  ))}
                </tr>
